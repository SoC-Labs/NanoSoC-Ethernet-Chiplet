...
**Date:** 2026-08-17 · **Scope:** the 5 "real" findings in `CDC_B7_BASELINE.md` §5,
...
| A4F | `role_locked_o` → `lock_thresh_sync2` (`link_rx_clk`) | Reset_sync02 | the deliberate design of `../design/RESET_ORDERING.md` §2 | **not a real finding** |
...
This is precisely the mechanism `../design/RESET_ORDERING.md` §2 exists to describe: the
...
Cross-check answer for `../design/RESET_ORDERING.md`: it covers the D2D/TideLink reset regimes
...
**R4 — Extend `../design/RESET_ORDERING.md` to cover the eth-subsystem.**
...
- **`axi_chiplet_controller` is still black-boxed.** Everything said here about
  TideLink's reset regimes is taken from `../design/RESET_ORDERING.md` and the RTL, not from a CDC
