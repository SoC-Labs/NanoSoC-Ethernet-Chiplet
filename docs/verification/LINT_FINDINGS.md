...
nets. The peer-aperture HREADY cycle (`../design/D2D_HREADY_LOOP.md`) passed elaboration
...
**Consequence.** `CDC_FINDINGS.md` carried the same error and used it to
defer the `CLKDMN` CDC sign-off to another host. That deferral is void — the
sign-off can run here. See `CDC_FINDINGS.md` §"SpyGlass availability".
...
`../design/D2D_HREADY_LOOP.md` option 2). A conservative static loop-finder cannot see the
...
| 2 | low | UNUSED | `nanosoc_eth_chiplet.sv:311` | `tcapb_paddr[11:8]` unused | **By design, mild over-provisioning.** The TideChart AHB→APB bridge is instantiated `ADDRWIDTH(12)` (4 KB) but the shim consumes only `[7:0]` (`APB_ADDR_W=8`, line 796). ~~*Optional cleanup:* `ADDRWIDTH(8)` would match the consumer and drop the warning.~~ **DO NOT NARROW — 2026-08-26.** Bit 8 is what the TideChart subtree block at 0x100-0x124 needs in order to decode. It is discarded at the shim boundary today, which is why 0x100-0xFFF aliases onto 0x000-0x0FF in BOTH directions, writes included, with `pslverr` tied 0. Narrowing the bridge would move the loss upstream and make it unfixable at the shim. The fix (`[8:0]`, `APB_ADDR_W=9`) is parked on `next/tidechart-apb-width`; it is deliberately NOT landed for the 1 September submission. |
...
mode `LINT_REMEDIATION_PLAN.md` §4 records: a rule-name waiver whose recorded
