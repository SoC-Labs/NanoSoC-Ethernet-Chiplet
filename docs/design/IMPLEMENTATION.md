...
arrived). `../verification/ELAB_STRICT_FINDINGS.md`.
...
| No combinational loops / latches / width bugs in our RTL | `make lint` (Verilator) | `../verification/LINT_FINDINGS.md` |
| **A memory transaction crosses between two REAL SoCs over the link** | `verif/g2_soc_pair` — die A `0x2F00_1000` → die B's real `shared_sram_0` `0x2D00_1000`, payload intact, CAM-off control | `../verification/G2_SOC_PAIR_STATUS.md` |
| **The data plane crosses BOTH ways** — peer read round-trip | `verif/g2_soc_pair` STAGE 2b — read back `0x2F00_1000` = written value across the link | `../verification/G2_SOC_PAIR_STATUS.md` |
| **Back-to-back bursts stay intact** | `verif/g2_soc_pair` STAGE 2c — 8-word write+read sequence, every beat verified | `../verification/G2_SOC_PAIR_STATUS.md` |
| The link trains between two real SoCs, firmware-free | same env, STAGE 1 | `../verification/G2_SOC_PAIR_STATUS.md` |
...
2. **Write-data alignment** (`d2d_ahb_m_hwdata_q`, 1-cycle delay). TideLink
   ...
   `../verification/G2_SOC_PAIR_STATUS.md` (Milestone 2 finding: RESOLVED).
...
| `../verification/G2_SOC_PAIR_STATUS.md` | the two-real-SoC proof + the write-data / read-pipe fixes |
| `../verification/CDC_FINDINGS.md` | the structural CDC pass + `constraints/nanosoc_eth_chiplet_cdc.sdc` for full sign-off |
| `../verification/ELAB_STRICT_FINDINGS.md` | the strict-elaboration gate (`make elab-strict`) — catches multi-drivers synthesis rejects |
| `../verification/LINT_FINDINGS.md` | lint tooling, the sanity harness, triaged findings |
...
- **CDC signoff**: a first structural CDC pass is done (`verif/cdc/run.sh`, HAL
  ...
  flow). The same SDC seeds the ASIC STA constraints. `../verification/CDC_FINDINGS.md`.
- **Lint**: `make lint` (Verilator) covers our wrapper RTL — no non-waived
  findings. `../verification/LINT_FINDINGS.md`.
...
- **Peer READ round-trip**: **fixed** (2026-07-10), and the fix is now UPSTREAM.
  ...
  `../verification/G2_SOC_PAIR_STATUS.md` "read round-trip".
