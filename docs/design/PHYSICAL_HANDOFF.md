...
window is ever widened. See `PEER_APERTURE_PROGRAMMING.md`.
...
- ~~**No transaction has ever crossed a die boundary in THIS integration**, in
  ...
  `make regress` gate and passes there. See `../verification/G2_SOC_PAIR_STATUS.md`.
...
- ~~**A combinational cycle on the peer aperture's `HREADY`.**~~ **FIXED
  ...
  was the only TideLink port with the dependence. See `D2D_HREADY_LOOP.md`.
...
- ~~**No lint or CDC signoff** has been run on the integrated top.~~ **PARTLY
  ...
  - **Lint: done and green.** `make lint` (Verilator) reports no non-waived
    ...
    *detects* the `hready` combinational cycle (UNOPTFLAT) against a deliberately
    ...
    check having quietly stopped looking. Findings log: `../verification/LINT_FINDINGS.md`.
  - **CDC: a first structural pass exists, and is NOT a clean bill.** `make cdc`
    ...
    `../verification/CDC_FINDINGS.md` says in its own words that it is "a **starting point** for
