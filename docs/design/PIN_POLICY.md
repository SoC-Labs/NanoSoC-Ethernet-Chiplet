...
> **STATUS — the "all pins on default branches" claim below is out of date.**
> Measured 2026-08-19 in this checkout: `git ls-tree HEAD` records `tidelink`
> `e6aaa82f` while the working tree has `3620af33` checked out (a live drift), and
> none of the three top-level submodules is currently on its default branch —
> `nanosoc-multicore-system` is on `dma250-ctxram-shrink`, `tidechart` on
> `fix/tidechart-h-dupdecl`, `tidelink` on `archive/2026-08-collapse/...`. Two more
> submodules (`ASIC/asic-flows`, `ASIC/asic-toolkit`) have been added since and are
> not covered below. The **policy** in this file still stands; the **2026-07-16
> measurement** does not. Re-run the chain check before quoting any row.
...
`../verification/G2_SOC_PAIR_STATUS.md` (`../verification/G2_PAIR_SIM.md` is the older design note).
