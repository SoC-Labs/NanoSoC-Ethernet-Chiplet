# 58 — Broker correspondence (deliberately not tracked)

[index](00-index.md) · [28 DRC status](28-drc-status-and-attribution.md)

> **Status — policy note, current.** `docs/tapeout/27-broker-questions-SEND-NOW.md` is
> gitignored and present only in a working checkout; verify with
> `git check-ignore -v docs/tapeout/27-broker-questions-SEND-NOW.md`.

The questions-to-the-broker document at `27-broker-questions-SEND-NOW.md` is **not tracked
in this repository**, and its absence is the point rather than an oversight.
