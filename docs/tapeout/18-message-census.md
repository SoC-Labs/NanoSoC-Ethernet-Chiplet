...
|---|---|---|---|---|---|
| `TECHLIB-302` | WARN | 111 | `No function defined for cell 'PVSS3DGZ_G'. The cell will only be used for analysis. (File $TSMC_65_HOME/.../tphn65lpgv2od3_slwc.lib)` | 111 IO/physical cells have no logic function, so they are timing-analysis-only. | BENIGN — supply, ESD, clamp and cut cells genuinely have no function. |
...
| `IMPSE-110` | **ERROR** | 3 | `File '<mentor install>/calibre/shared<icv install>/tools/queryenc/encounter.tcl' line 47: can't find package Tk 8.0.` | The Calibre-in-Innovus integration needs Tk, which is absent in batch. | REAL-BUT-KNOWN — `16-open-defects.md`; fixed by `env -u CALIBRE_HOME`. |
