...
Classified by the paths each commit touches (script in §7). The class *is* the verdict.
...
## 8. Documented claims that are now false

Named, with the document and the measurement that falsifies each.

### 8.1 `docs/tapeout/00-index.md:17` — the vendor guard's path and trigger
...
### 8.2 "No Tempus or PrimeTime installed" — `ci/signoff.yaml`, gap `sta-signoff`
...
### 8.3 "Signoff STA ran with ZERO parasitics"
...
### 8.4 "The LEC chain joint is broken" (the 08-08 vintage)
...
### 8.5 "The shipping GDS is `full-20260814`"
...
### 8.6 "The boot ROMs are verified at the reticle in `full-20260814`"
...
### 7.1 Which tree level each number came from
...
### 7.3 Reproducing the commit classification in §6
...
### 7.4 What I did NOT measure, recorded as unknown
...
### 7.5 Standing hazards for the next session
