# 60 — `pgfix-A`: the provenance set, stated before the run

> **Status — the run did launch, and it got as far as PLACE only.** Verified on disk
> 2026-08-19: `build/pgfix-A/reports/` holds `place_*.rep` and `fp_pg_check.rep` written
> 2026-08-18 22:55–22:58, and there is no CTS, route or stream stage. **Every file in
> `build/pgfix-A/outputs/` — including `nanosoc_eth_chiplet_pads.gds` — is a symlink into
> `build/full-20260814/outputs/`**, so anything calling itself "the pgfix-A GDS" is
> `full-20260814` bytes. Its `inputs/` and `scripts/` are also symlinks, into a session
> scratchpad outside the repository.

**Status when written:** staged, not launched. Two invocations sat on permission denials
