...
| 5 | TideChart fabric | election/enum/hotplug event | `TC_STATUS` election_done/enum_done edge; `TC_HOTPLUG` sticky | `d2d_irq[14]` → CPU1 IRQ15 | Pulse on election/enum edges. **Dead until the TideChart RTL fix** (P4: dual-root, reset doesn't clear — see [`TIDECHART_TEST_PLAN.md`](TIDECHART_TEST_PLAN.md)). |
