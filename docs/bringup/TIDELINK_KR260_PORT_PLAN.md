...
| `chiplet_d2d_decode` | **reuse as-is** | — | pure RTL, board-agnostic; already lint/sim-clean (`../verification/LINT_FINDINGS.md`, `../verification/G2_PAIR_SIM.md`). |
...
| Pad ring / boundary (`../design/PIN_POLICY.md`, `../design/PIN_MAP.md`, unbond pads, `../design/POWER_DOMAINS.md`) | **not used** (FPGA drives real device I/O via XDC) | **ASIC-only** | the 23-pad budget, tie/open policy, power domains are tapeout concerns. |
...
2. **`ahb_mng`→SoC terminus in the FPGA target.** The chiplet top wires it, but
   ...
   `d2d_ahb_s` HREADY-loop discipline (`../design/D2D_HREADY_LOOP.md`) rather than the
   old BRAM loopback.
...
7. **nanosoc UART console path.** The KR260's micro-USB (FTDI FT4232) is
   ...
   `../design/PIN_MAP.md` §5c) and have no path to that FTDI, so they will **not**
...
- **M0 (sim smoke):** best executed in the **tidelink** repo's cocotb harness
  ...
  a `nanosoc_eth_chiplet` pair sim here (this repo already has `../verification/G2_PAIR_SIM.md`
  / `../verification/G2_SOC_PAIR_STATUS.md` for a SoC-pair TB — the natural home for a
