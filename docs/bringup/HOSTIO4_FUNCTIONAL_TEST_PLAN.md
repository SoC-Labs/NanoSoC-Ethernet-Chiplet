...
> | `0x29000000` | D | `0x00000004` | **boot gate bit 2 already SET** — §5.5 / §6.3, still unresolved |
...
> ## `[R2]` FINDING — THE PHC DOES NOT KEEP REAL TIME. IT RUNS 10× SLOW.
>
> HIO-404, measured:
>
> ```
...
> fabric HCLK         =  ~100,000,000   <- measured independently at the perf probe (§1.4)
> ```
>
> At `NS_INCR = 4`, real time requires a **250 MHz** core. The implied clock is **~25 MHz**, and the fabric measures **~100 MHz**. **So either the PHC's clock source or its increment is wrong by a factor of ten** — and a PTP hardware clock advancing at a tenth of real time cannot run a servo, cannot timestamp usefully, and cannot align a PPS.
>
> **Three reasons this is a finding and not a bad test constant.**
> 1. The `implied_core_hz / fabric_hz` ratio is **~0.25**. HIO-404's own escape hatch — *if the ratio is ~1.0 the PHC is fine and the expected-frequency constant is wrong* — **does not apply**. The ratio being wrong is precisely what rules out the benign explanation, and it is why that cross-check was worth building in.
> 2. `NS_INCR` was **read back from the die**, not assumed. The 10× is not an arithmetic error in the expectation.
> 3. The fabric rate it is measured against was established **independently, twice** — three SNAP deltas agreeing to ~0.02 %, and the `0x05F5E100` word at `0x18000000`.
>
> **This is the FIRST hardware measurement of the PTP subsystem on this part.** It was on the never-tested list until now, so there is no prior baseline to say whether this is a regression or has always been true. **The clock domain is still unestablished:** resolve whether the PHC is fed a divided clock or whether `NS_INCR` is mis-strapped before naming a cause — the two are indistinguishable from this port, because ADP cannot see the PHC's clock and `NS_INCR` is `B`-class besides (its chiplet reset value was never located, §0).
