...
(`docs/LINT_REMEDIATION_PLAN.md` §4); and a merged unit-level ruleset switched
...
`docs/LINT_REMEDIATION_PLAN.md` §4 records that `make cdc` runs top
...
- the `UNOPTFLAT` is inside `u_wlink.axi2wl.wlink_axiawFC.ack_nack_fifo`, i.e.
  ...
  worth reading against `docs/CROSS_DIE_WEDGE_ROOTCAUSE.md` before the next
