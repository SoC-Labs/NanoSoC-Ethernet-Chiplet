...
#
# HOW A CLEAN REPORT IS RECOGNISED. Innovus writes the `Total Violations` trailer
# ONLY when there are violations; a clean report is the header fence and the
# sentence `No DRC violations were found`, ten lines in total. A gate that
# cross-checks itself against the trailer therefore has nothing to check against
# on exactly the run that deserves to pass, and hard-fails it - which is what
# happened to gdsrun-20260826-rc1 under the toolkit's copy of this parser on
# 2026-08-26.
#
# THE FIX IS NOT "A MISSING TRAILER MEANS ZERO". `declared` carries the report's
# OWN total and `attested` says where it came from: the trailer, or a clean
# report positively identified from its header fence, a `Generated by:` naming
# Innovus, a `Command:` line beginning check_drc, and exactly one sentinel with
# no violation record to contradict it. A file with neither attestation stays
# UNVERIFIED, which is what a truncated or absent report must remain.
#
# Kept deliberately identical in behaviour to route_drc_census in the toolkit's
# flow/innovus/4_route.tcl, which is the copy the shipping runs execute.
proc evroute_drc_census {path _c} {
    ...
    array set c {total 0 trailer -1 special 0 regular 0 pin_only 0 \
                 ...
                 header 0 tool {} cmd {} design {} clean 0 declared -1 attested {}}
    ...
    while {[gets $fh line] >= 0} {
        ...
        # The header, which is what makes an absent measurement distinguishable
        # from a measured zero. Every line of it starts `#`, so none of it can
        # reach the `^([A-Za-z]+):` record match below.
        if {[regexp {^#####+\s*$} $line]} { incr c(header) ; continue }
        if {[regexp {^#\s*Generated by:\s*(.+?)\s*$} $line -> v]} { set c(tool) $v ; continue }
        if {[regexp {^#\s*Command:\s*(.+?)\s*$} $line -> v]}      { set c(cmd) $v ; continue }
        if {[regexp {^#\s*Design:\s*(.+?)\s*$} $line -> v]}       { set c(design) $v ; continue }
        # Anchored: the phrase inside a record or a note is not the tool
        # declaring the design clean.
        if {[regexp {^\s*No DRC violations were found\.?\s*$} $line]} {
            incr c(clean) ; continue
        }
    ...
    close $fh
    # What the report says about ITSELF, which is a different question from what
    # this parser counted. Conjunctive on purpose: drop any single conjunct and
    # an absent measurement becomes indistinguishable from a measured zero.
    if {$c(trailer) >= 0} {
        set c(declared) $c(trailer)
        set c(attested) trailer
    } elseif {$c(header) >= 2 && $c(clean) == 1 && $c(total) == 0
              && [string match -nocase {*innovus*} $c(tool)]
              && [regexp {^check_drc($|\s)} $c(cmd)]} {
        set c(declared) 0
        set c(attested) clean-sentinel
    }
    set out {}
...
if {$EVR_CHECKS} {
    if {!$have_drc} {
        ...
        # A report with a header and nothing else parses as total 0, owned 0 and
        # passes every test below - a clean bill from a file that said nothing.
        # pnr_assert_file cannot catch it: a header alone is non-zero bytes. So
        # the report must SAY its own total, and this parser must agree with it.
        #
        # `attested` and not `trailer < 0`: Innovus writes no trailer on a clean
        # design, so keying on the trailer made a perfect run unreportable. See
        # evroute_drc_census - the replacement is a stronger test, not a weaker
        # one.
        if {$DRCF(attested) eq ""} {
            lappend hard "the check_drc report declares no total of its own -\
                          no 'Total Violations' trailer, and no attested\
                          'No DRC violations were found' either (header fence\
                          $DRCF(header), tool '$DRCF(tool)', command\
                          '$DRCF(cmd)', $DRCF(clean) sentinel(s),\
                          $DRCF(total) record(s) parsed). Its count of\
                          $DRCF(total) is not evidence of anything. Read\
                          $REPORT_DIR/${block_name}_imp_drc.rep by hand."
        } elseif {$DRCF(declared) != $DRCF(total)} {
            lappend hard "DRC parser counted $DRCF(total) but the report\
                          declares $DRCF(declared) (by $DRCF(attested)) - the\
                          parser is wrong, do not trust either number"
        }
        if {$DRCF(clean) > 0 && ($DRCF(total) > 0 || $DRCF(trailer) > 0)} {
            lappend hard "the check_drc report CONTRADICTS ITSELF: $DRCF(clean)\
                          'No DRC violations were found' sentinel(s) in a file\
                          that also carries $DRCF(total) violation record(s)\
                          and a trailer of $DRCF(trailer)."
        }
