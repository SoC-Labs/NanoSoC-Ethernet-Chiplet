...
# 5. A report with markers but NO trailer. trailer must stay -1 AND the report
#    must attest to nothing, so the gate can tell "zero violations" from "the
#    report never finished" - the distinction that separates a clean run from a
#    truncated one.
set r5 "SPACING: ( ParallelRunLength Spacing ) Special Wire of Net VDD & Blockage of Cell u_z  ( M4 )
Bounds : ( 1.0, 2.0 ) ( 3.0, 4.0 )"
array set e5 {total 1 trailer -1 special 1 regular 0 attested {} declared -1}
case "truncated report (no trailer)" $r5 e5

## ---------------------------------------------------------------------------
## 5a-5e. THE SHAPE INNOVUS ACTUALLY WRITES WHEN A DESIGN IS CLEAN.
##
## Case 1 above ("  Total Violations : 0 Viols.") is an INVENTED specimen: the
## tool does not write a trailer when the count is zero. What it writes is the
## ten-line report below, verbatim off
## gdsrun-20260826-rc1/reports/nanosoc_eth_chiplet_pads_imp_drc.rep with the
## absolute path shortened. Keying the gate on the trailer therefore made a
## clean run UNREPORTABLE, and on 2026-08-26 it hard-failed one.
##
## These five arms are the both-direction proof of the replacement. 5a is the
## only one that may attest; 5b-5e each remove exactly one attesting element and
## must all stay unattested, or the fix has decayed into "a missing trailer
## means zero" - which is the defect it replaced.
set CLEAN_HDR "###############################################################
#  Generated by:      Cadence Innovus 21.11-s130_1
#  OS:                Linux x86_64(Host ID srv03335)
#  Generated on:      Wed Aug 26 05:22:14 2026
#  Design:            nanosoc_eth_chiplet_pads
#  Command:           check_drc -limit 200000 -out_file reports/imp_drc.rep
###############################################################"

array set e5a {total 0 trailer -1 declared 0 attested clean-sentinel clean 1 header 2}
case "REAL clean report (no trailer)" "$CLEAN_HDR\n\nNo DRC violations were found" e5a

array set e5b {total 0 trailer -1 declared -1 attested {} clean 0 header 2}
case "clean report cut before sentinel" "$CLEAN_HDR\n" e5b

array set e5c {total 0 trailer -1 declared -1 attested {} clean 1 header 0}
case "sentinel with no header" "No DRC violations were found" e5c

set WRONG_CMD [string map {"check_drc -limit 200000 -out_file reports/imp_drc.rep" \
                           "check_connectivity -type regular"} $CLEAN_HDR]
array set e5d {total 0 trailer -1 declared -1 attested {} clean 1}
case "sentinel from the wrong command" "$WRONG_CMD\n\nNo DRC violations were found" e5d
...
Bounds : ( 1.0, 2.0 ) ( 3.0, 4.0 )
  Total Violations : 1 Viols."
array set e5e {total 1 trailer 1 declared 1 attested trailer clean 1 regular 1}
case "report contradicting itself" $r5e e5e
