...
#
# EVERYTHING BELOW IS A DESIGN FACT THE TOOLKIT CANNOT INFER, and each one that
# were left unset would cost exactly the metrics that depend on it - rendered as
# NOT MEASURED, never as a plausible default.
...
# links NINE libraries - seven memory compilers, the IO pads, and this. Picking
...
# Source: build/full-20260814/logs/syn_lib_check.log, `Technology libraries`.
DESIGN_REPORT_STDCELL_LIB := tcbn65lpwc

# The gate-equivalent unit. Its AREA IS READ FROM THE REPORT (1.44 um2 here,
# 8742.240 over 6071 instances), never typed, so the ratio always uses this
# library's own number rather than a remembered one from another node.
DESIGN_REPORT_NAND2_CELL := ND2D1

# Core box inset from each die edge, in um. The ONE piece of floorplan geometry
# the flow cannot derive: create_floorplan takes the core-to-IO clearance but
# the pad height comes from the IO cell's LEF.
#
#     135  the tphn65lpgv2od3 pad cells' LEF SIZE height
#    + 70  CORE_TO_IO, per ../genus-innovus/scripts/floorplan.tcl:82
#    = 205 giving a core box of (205,205)-(1395,1795) = 1190 x 1590 um
#
# KEEP THIS IN STEP WITH floorplan.tcl's CORE_TO_IO. That file's own comments
# record the two occasions the value moved (50 -> 70) and what it cost; if it
# moves again this is a second copy that will not notice.
